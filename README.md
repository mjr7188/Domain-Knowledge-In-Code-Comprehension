...
better predicts comprehension of domain-specific pseudocode across three domains:
Pharmacology, Chemistry, and Finance. Ten participants completed the survey in full,
split into a computing group (n=3) and a non-computing group (n=7). Comprehension
was measured via domain-specific question sets embedded in the survey instrument
and routed based on self-reported familiarity thresholds.
...
| `scipy` | Spearman correlation, Mann-Whitney U, t and F distributions |
...
> whether any per-question response column for that domain is non-blank.
...
coursework*), confirming they were actually routed to that domain's tasks
by the survey instrument.
 
> **Finance** fails this check — all 7 non-computing participants reported
> only *General awareness* (score = 1) and were therefore never presented
> with the finance comprehension tasks. Finance is excluded from all
> inferential analyses.
 
### Stage 3 — Analysis 1: Spearman Correlation
Spearman's *r* between domain familiarity score and comprehension score,
computed across **all participants** for each eligible domain.
`scipy.stats.spearmanr` is used with `nan_policy='omit'`.
 
### Stage 4 — Analysis 2: Mann-Whitney U
Two-sided Mann-Whitney U test comparing per-domain comprehension scores between:
- **CS group** — participants in Computing majors (n=3)
- **Non-CS group** — all non-computing participants routed to that domain's tasks
Per-domain, only participants who were **routed** to that domain's tasks
(i.e., have at least one non-blank response in the domain's question columns)
are included. The Computing domain is always included as a shared unfamiliar
baseline for non-computing participants regardless of eligibility.
...
Only participants **routed to the pharmacology tasks** are included (n=8).
Standard errors, t-statistics, p-values, R², adjusted R², and the overall
F-statistic are computed manually from the OLS residuals and reported in full.
...
| Spearman *r* | Finance | excluded (eligibility check) |
| Mann-Whitney U | Pharmacology | U = 8.5, p = 0.878 (n.s.) |
| Mann-Whitney U | Chemistry | U = 10.5, p = 0.421 (n.s.) |
| Mann-Whitney U | Computing | U = 20.0, p = 0.035 * |
| Mann-Whitney U | Finance | excluded (eligibility check) |
| Regression (n=8) | Pharmacology SC1 | F(2,5) = 0.625, p = 0.573 (n.s.) |
| Regression (n=8) | Domain familiarity | coeff = 0.572, p = 0.416 (n.s.) |
| Regression (n=8) | Programming experience | coeff = 0.657, p = 0.320 (n.s.) |
 
The only statistically significant result (α = 0.05) is the Mann-Whitney U
comparison on the Computing domain (p = 0.035), indicating that computing
students comprehended computing pseudocode more accurately than non-computing
students. No significant relationship was found between domain familiarity
and comprehension accuracy in either eligible non-computing domain. All
regression results were non-significant, consistent with insufficient
statistical power at n=8 with two predictors.
